...
function preTableBlog_tags(){
      ...
      $('.table').on("click",".ordemUp",function(e){ 
          var params = { idblog_tags: $(this).attr("codigo")}
          $.post( 'blog_tags_script.php?opx=alteraOrdemCima', params,
                function(data){  
    ...
    $('.table').on("click", ".ordemDown", function(e){  
        var params = {idblog_tags: $(this).attr("codigo")}  
        $.post( 'blog_tags_script.php?opx=alteraOrdemBaixo', params,
         function(data){  
            var resultado = new String (data.status);  
            if(resultado.toString() == 'sucesso'){ 
              dataTableBlog_tags(); 
    ...
    $('.table').on("click", ".inverteStatus", function (e) {
        var params = {
            idblog_tags: $(this).attr("codigo")
        }

        $.post(
            'blog_tags_script.php?opx=inverteStatus',
            params,
            function (data) {
                var resultado = new String(data.status);
...
var myColumnDefs = [
	{key:"idblog_tags", sortable:true, label:"ID", print:true, data:true},
          ...
          success:function(data){
                ...
                if(data.totalRecords > 0){
                    ...
                    $.each(myColumnDefs, function(col, ColumnDefs){
                      ...
                      tr += '<a href="index.php?mod=blog_tags&acao=formBlog_tags&met=editBlog_tags&idu='+value.idblog_tags+'"><img src="images/ico_edit.png" height="16" width="16" alt="ico" /><div class="tt"><span class="one">Editar</span><span class="two"></span></div></a>';
                      tr += '<a href="#" onclick="wConfirm(\'Excluir Registro\',\'Tem certeza que deseja excluir o registro '+value.idblog_tags+' ?\',\'php\', \'blog_tags_script.php?opx=deletaBlog_tags&idu='+value.idblog_tags+'\');"><img src="images/ico_del.png" height="17" width="17" alt="ico" /><div class="tt"><span class="one">Excluir</span><span class="two"></span></div></a>';
                      tr += '</div></td>';
                    }); 
...
$(document).ready(function(){
    ...
    $(".cancelar").click(function(event){
        location.href='index.php?mod=blog_tags&acao=listarBlog_tags';
    });
    ...
    $(".confirm").click(function(){
        excluirImagem('blog_tags', tipo, img, thisElem);
...
function verificaUrlrewrite(url) {
    ...
    $.ajax({
        url: 'blog_tags_script.php',
        dataType: 'json',
        data: "opx=verificarUrlRewrite&idblog_tags=" + id + "&urlrewrite=" + url,
        type: 'post',
        beforeSend: function () {
            Loader.show();
        },
