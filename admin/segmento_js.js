...
function preTableSegmento(){
      ...
      $('.table').on("click",".ordemUp",function(e){ 
          var params = { idsegmento: $(this).attr("codigo")}
          $.post( 'segmento_script.php?opx=alteraOrdemCima', params,
                function(data){  
    ...
    $('.table').on("click", ".ordemDown", function(e){  
        var params = {idsegmento: $(this).attr("codigo")}  
        $.post( 'segmento_script.php?opx=alteraOrdemBaixo', params,
         function(data){  
            var resultado = new String (data.status);  
            if(resultado.toString() == 'sucesso'){ 
              dataTableSegmento(); 
...
var myColumnDefs = [
	{key:"idsegmento", sortable:true, label:"ID", print:true, data:true},
          ...
          success:function(data){
                ...
                if(data.totalRecords > 0){
                    ...
                    $.each(myColumnDefs, function(col, ColumnDefs){
                      ...
                      tr += '<a href="#" onclick="wConfirm(\'Excluir Registro\',\'Tem certeza que deseja excluir o registro '+value.idsegmento+' ?\',\'php\', \'segmento_script.php?opx=deletaSegmento&idu='+value.idsegmento+'\');"><img src="images/ico_del.png" height="17" width="17" alt="ico" /><div class="tt"><span class="one">Excluir</span><span class="two"></span></div></a>';
                      tr += '</div></td>';
                    }); 
