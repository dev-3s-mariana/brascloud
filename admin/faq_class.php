<?php 
	...
	function cadastroFaq($dados)
	{
		...
		$sql = "INSERT INTO faq(pergunta, resposta, status, ordem, idsuporte) VALUES (
						'".$dados['pergunta']."',
						'".$dados['resposta']."',
						'".$dados['status']."',
						'".$dados['ordem']."',
	...
	function buscaFaq($dados = array())
	{
		...
		if (array_key_exists('idfaq',$dados) && !empty($dados['idfaq']) )
			$buscaId = ' and f.idfaq = '.intval($dados['idfaq']).' '; 

        //busca pelo id
        $buscaIdsuporte = '';
        if (array_key_exists('idsuporte',$dados) )
		...
		$sql = "SELECT $colsSql, s.titulo as nome_suporte $buscaMax FROM faq as f LEFT JOIN suporte as s ON f.idsuporte = s.idsuporte WHERE 1  $buscaId $buscaIdsuporte $buscaPergunta $buscaOrdem $buscaRespota $buscaStatus $orderBy $buscaLimit ";
