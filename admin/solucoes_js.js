		...
		success: function (data) {
			...
			if (data.totalRecords > 0) {
				$.each(data.records, function (index, value) {
					...
					tr += '<a href="#" onclick="wConfirm(\'Excluir Registro\',\'Tem certeza que deseja excluir o registro ' + value.nome + ' ?\',\'php\', \'solucoes_script.php?opx=deletaSolucoes&idu=' + value.idsolucoes + '\');"><img src="images/ico_del.png" height="17" width="17" alt="ico" /><div class="tt"><span class="one">Excluir</span><span class="two"></span></div></a>';
					tr += '</div></td>';
				});
...
function carregaIconeAcao() {
   $('#icone_pai').on("click", ".icone_icone", function (e) {
      ...
      imgRec = $(".img-upload.img-"+grid+"-"+key).attr('src');
      imgRec = imgRec.replace('files/recursos/', '');

      $(".img-upload.img-"+grid+"-"+key).attr('src','https://via.placeholder.com/50?text=Upload+Foto');
      $(".file-upload.upload-"+grid+"-"+key).val('');
      $(".file-upload.upload-"+grid+"-"+key).siblings('.nome-img-cadastrada').val('');
   });
}
...
$(document).ready(function () {
    ...
    $(".excluir-arquivo").click(function(e){
      ...
      $.ajax({
         ...
         type: "POST",
         dataType: "json",
         data: {opx: 'excluirArquivo', idsolucoes: $("#idsolucoes").val(), arquivo: $(thisElement).attr('data-arquivo')},
         beforeSend: function () {
            Loader.show();
         },
         success: function (data) {
   ...
   $("#formSolucoes").on("click", ".btn-choose-icon", function(){
   ...
   //===== Grid itens =====//
      $(".btn-recursos").on('click', function () {
         addGrid('recursos', true, 81, 81);
      });

      $(".btn-testes").on('click', function () {
         addGrid('testes', false);
      });

      $(".btn-diferenciais").on('click', function () {
         addGrid('diferenciais', false, 0, 0, false);
      });
      ...
      // });
        ...
        $(document).on("click", ".excluirRecursos", function () {
            ...
            $('#excluirRecursoRecursos-'+key).val('0');
        ...
        $(document).on("click", ".excluirTestes", function () {
            ...
            $('#excluirRecursoTestes-'+key).val('0');
            // $('.removeTestes-'+key).remove();
        });

        $(document).on("click", ".subirDiferenciais", function () {
            var thisDataKey = $(this).parent().parent().parent().attr('data-key');
            var auxDiv = $('.div-aux');
            var thisTr = $(this).parent().parent().parent();
            var prevTr = $(thisTr).prevAll('.box-diferenciais').first();
            var thisInputPos = $(thisTr).find('.td-flex').find('input');
            var prevInputPos = $(prevTr).find('.td-flex').find('input');
            var auxInputPos = $(thisInputPos).val();
            if($(prevTr).length > 0){
        ...
        $(document).on("click", ".descerDiferenciais", function () {
            var thisDataKey = $(this).parent().parent().parent().attr('data-key');
            var auxDiv = $('.div-aux');
            var thisTr = $(this).parent().parent().parent();
            var nextTr = $(thisTr).nextAll('.box-diferenciais').first();
            var thisInputPos = $(thisTr).find('.td-flex').find('input');
            var nextInputPos = $(nextTr).find('.td-flex').find('input');
            var auxInputPos = $(thisInputPos).val();
            if($(nextTr).length > 0){
        ...
        $(document).on("click", ".excluirDiferenciais", function () {
            var key = $(this).parent().parent().parent().attr('data-key');
            $('.removeDiferenciais-'+key).hide();
            // $('.removeDiferenciais-'+key).next('tr').remove();
            $('#excluirRecursoDiferenciais-'+key).val('0');
            // $('.removeDiferenciais-'+key).remove();
        });
    //===== Fim grid itens =====//
   ...
   ////////////////////////////////////////////////
    ...
    $('.save-cropped-image').click(function(e){
        ...
        formData.append("idsolucoes", $('input[name=idsolucoes]').val());
        if(dataImageType == 'imagem'){
            ...
            success:function(data){
                ...
                if(data.status == true){
                    if(dataImageType == 'imagem'){
                        ...
                        $('#select-image-1 .img-solucoes-form').attr('src', 'files/solucoes/'+data.imagem);
                        ...
                        $('#select-image-2 .img-solucoes-form').attr('src', 'files/solucoes/'+data.imagem);
    ...
    $(".cancelar").click(function(event){
        ...
        if($("#imagem-value").val() == '' && $("#imagem_2-value").val() == ''){
            location.href='index.php?mod=solucoes&acao=listarSolucoes';
        }
        else{
            if($("#idsolucoes").val() == 0){
                if($("#imagem-value").val() != ''){
                    cancelarImagem('solucoes', $("#imagem-value").val());
                }
    ...
    $(".confirm").click(function(){
        excluirImagem('solucoes', tipo, img, thisElem);
